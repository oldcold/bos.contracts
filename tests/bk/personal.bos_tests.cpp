...
BOOST_FIXTURE_TEST_CASE(set_personal_and_homepage, personal_bos_tester)
try
{
   //only owner can call setpersonal
   BOOST_REQUIRE_EQUAL(error("missing authority of alice"),
                       push_action(N(bob), N(setpersonal), mvo()("account", "alice")("key", "favorfood")("value", "pizza")));

   //value length check
   string long_value;
   long_value.assign(1025, 'a');
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("value should be less than 1024"),
                       push_action(N(alice), N(setpersonal), mvo()("account", "alice")("key", "somekey")("value", long_value)));

   //setpersonal should success,and check result
   BOOST_REQUIRE_EQUAL(success(),
                       push_action(N(alice), N(setpersonal), mvo()("account", "alice")("key", "favorfood")("value", "pizza")));
   ...
   BOOST_REQUIRE_EQUAL(success(),
                       push_action(N(alice), N(setpersonal), mvo()("account", "alice")("key", "favorfood")("value", "noodles")));
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("url is too long"),
                       push_action(N(alice), N(sethomepage), mvo()("account", "alice")("url", long_url)));

   //url prefix check
   string url = "abcdefg";
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("illegal url"),
                       push_action(N(alice), N(sethomepage), mvo()("account", "alice")("url", url)));

   //sethomepage should success,and check result
   BOOST_REQUIRE_EQUAL(success(),
                       push_action(N(alice), N(sethomepage), mvo()("account", "alice")("url", "http://somepage.com")));
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("illegal url"),
                       push_action(N(alice), N(setpersonal), mvo()("account", "alice")("key", "homepage")("value", url)));
   BOOST_REQUIRE_EQUAL(success(),
                       push_action(N(alice), N(setpersonal), mvo()("account", "alice")("key", "homepage")("value", "https://newpage.com")));
   data = get_personal(N(alice), "homepage");
