...
void pegtoken::create(symbol sym, name issuer, name address_style)
{
    ...
    eosio_assert(symbol_table.find(sym_raw) == symbol_table.end(), "token with symbol already exists in symbols table");
    ...
    eosio_assert(info_table.find(sym_raw) == info_table.end(), "token with symbol already exists in infos table");
    ...
    eosio_assert(limit_table.find(sym_raw) == limit_table.end(), "token with symbol already exists in limits table");
...
void pegtoken::setissuer(symbol_code sym_code, name issuer)
{
    ...
    eosio_assert(iter != info_table.end(), "token is not existed in infos table");
                         ...
                         name auditor, string remote_trx_id, asset quantity, uint64_t index, string memo)
{
    ...
    eosio_assert(iter_cast != cast_table.end() && iter_cast->to_account == to_account && iter_cast->to_address == to_address && iter_cast->remote_trx_id == remote_trx_id && iter_cast->remote_index == index && iter_cast->state == cast_state::CAST_INIT && iter_cast->quantity == quantity, "invalid cast");
                          ...
                          name auditor, string remote_trx_id, asset quantity, uint64_t index, string memo)
{
    ...
    auto index_str = remote_trx_id + std::to_string(index);
    auto iter_cast = cast_table.find(hash64(index_str));
    eosio_assert(iter_cast != cast_table.end() && iter_cast->to_account == to_account && iter_cast->to_address == to_address && iter_cast->remote_trx_id == remote_trx_id && iter_cast->remote_index == index && iter_cast->quantity == quantity && iter_cast->state == cast_state::CAST_INIT, "invalid cast");
                           ...
                           string remote_trx_id, uint64_t id, uint64_t remote_index, string memo)
{
    ...
    bool found = false;
    if (((melt_iter->enable == true && melt_iter->need_check == true) || melt_iter->need_check == false) && melt_iter->state == melt_state::WITHDRAW_INIT)
    {
