...
namespace bos {
    ...
    void bosio_issuance::timechecking() {
        ...
        print("today: ", today);
        print("_curday.today", _curday.today);
...
//        eosio_assert(false, "Hello Transfer");
        ...
        print("transfer.quantity:", transfer.quantity);
...
//        TODO: verify_maximum_supply in pegTokend & extended_asset
        ...
        }else if(transfer.quantity.symbol == symbol("BOS", 4) ){;
                std::tuple<uint64_t, uint64_t> tpsymday = validaterefundmemo(sender, transfer.memo);
                uint64_t symraw = std::get<0>(tpsymday);
                uint64_t day = std::get<1>(tpsymday);
...
#define EOSIO_DISPATCH_EX(TYPE, MEMBERS)                                            \
  ...
  void apply(uint64_t receiver, uint64_t code, uint64_t action) {              \
    ...
    auto self = receiver;                                                      \
    if (code == receiver || ((code == "eosio.token"_n.value || code == "bos.btc"_n.value  || code == "bos.eos"_n.value || code == "bos.eth"_n.value) && action == "transfer"_n.value)  || action == "onerror"_n.value  ) {      \
